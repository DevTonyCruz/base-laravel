...
<div class="col-lg-6">
    <div class="card">
        <div class="card-body">
            ...
            <form class="form-horizontal" action="{{ url('admin/categories/' . $category->id) }}" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-9">
                        <select class="form-control{{ $errors->has('parent_id') ? ' is-invalid' : '' }} select2" id="parent_id" name="parent_id" data-toggle="select2">
                            <option value="S">Seleccionar</option>
                            @foreach ($categories as $category_a)
                            <option value="{{ $category_a->id }}" {{ ($category->parent_id == $category_a->id ? "selected":"") }}>{{ $category_a->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-9">
                        <input type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" id="name" name="name" placeholder="Ingrese el nombre de la categoría" value="{{ $category->name }}" required>
                        @if ($errors->has('name'))
                            <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group row mb-3">
                    ...
                    <div class="col-9">
                        <textarea type="text" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" id="description" name="description" placeholder="Ingrese una descripción">{{ $category->description }}</textarea>
